		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="合计" prop="heji" >
					<el-input v-model.number="ruleForm.heji" placeholder="合计" clearable  :readonly="ro.heji"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="合计" prop="heji" >
				...
				<el-form-item class="date" v-if="type!='info'" label="发货时间" prop="fahuoshijian" >
					<el-date-picker
						value-format="yyyy-MM-dd HH:mm:ss"
						v-model="ruleForm.fahuoshijian" 
						type="datetime"
						:readonly="ro.fahuoshijian"
						placeholder="发货时间"
					></el-date-picker>
				</el-form-item>
				<el-form-item class="input" v-else-if="ruleForm.fahuoshijian" label="发货时间" prop="fahuoshijian" >
					<el-input v-model="ruleForm.fahuoshijian" placeholder="发货时间" readonly></el-input>
				</el-form-item>
			...
			<el-form-item v-if="type!='info'"  label="物流信息" prop="wuliuxinxi" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
					v-model="ruleForm.wuliuxinxi" 
					class="editor"
					myQuillEditor="wuliuxinxi"
					action="file/upload">
				</editor>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.wuliuxinxi" label="物流信息" prop="wuliuxinxi" >
				<span class="text ql-snow ql-editor" v-html="ruleForm.wuliuxinxi"></span>
			</el-form-item>
...
<script>
	...
	export default {
		...
		created() {
			this.ruleForm.fahuoshijian = this.getCurDateTime()
		},
		methods: {
			// 下载
			download(file){
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='fahuoshijian'){
							this.ruleForm.fahuoshijian = obj[o];
							this.ro.fahuoshijian = true;
							continue;
						}
						if(o=='wuliuxinxi'){
					...
					this.ruleForm.dingdanzhuangtai = '待确认'; 				}
				// 获取用户信息
				this.$http({
				...
				this.dingdanzhuangtaiOptions = "已确认,待确认".split(',')
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.wuliuxinxi = this.ruleForm.wuliuxinxi.replace(reg,'../../../springbootu4nad998/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
