	...
	export default {
		data() {
			...
			return {
				...
				rules: {
					jingxiaoshanghao: [
						{ required: true, message: '经销商号不能为空', trigger: 'blur' },
					],
					mima: [
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `jingxiaoshang/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.jingxiaoshangCrossAddOrUpdateFlag = false;
											this.parent.search();
