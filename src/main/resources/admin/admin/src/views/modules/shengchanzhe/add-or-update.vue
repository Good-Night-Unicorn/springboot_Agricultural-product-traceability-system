	...
	export default {
		data() {
			...
			return {
				...
				rules: {
					shengchanzhehao: [
						{ required: true, message: '生产者号不能为空', trigger: 'blur' },
					],
					mima: [
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `shengchanzhe/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.shengchanzheCrossAddOrUpdateFlag = false;
											this.parent.search();
