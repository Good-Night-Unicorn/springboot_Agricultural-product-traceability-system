	...
	export default {
		data() {
			...
			return {
				...
				rules: {
					jianguanzhezhanghao: [
						{ required: true, message: '监管者账号不能为空', trigger: 'blur' },
					],
					mima: [
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `jianguanzhe/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.jianguanzheCrossAddOrUpdateFlag = false;
											this.parent.search();
