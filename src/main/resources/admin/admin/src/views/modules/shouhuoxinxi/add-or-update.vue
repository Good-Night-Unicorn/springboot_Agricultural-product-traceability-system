	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-else-if="ruleForm.querenshijian" label="确认时间" prop="querenshijian" >
					<el-input v-model="ruleForm.querenshijian" placeholder="确认时间" readonly></el-input>
				</el-form-item>
			</template>
...
<script>
	...
	export default {
		...
		created() {
			this.ruleForm.querenshijian = this.getCurDateTime()
		},
		methods: {
			// 下载
			download(file){
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `shouhuoxinxi/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.shouhuoxinxiCrossAddOrUpdateFlag = false;
											this.parent.search();
